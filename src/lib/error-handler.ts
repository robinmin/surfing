...
class AuthErrorHandler {
    ...
    private errorMessages: Map<
        AuthErrorType,
        { message: string; userMessage: string; suggestion?: string }
    > = new Map();
    ...
    private initializeErrorPatterns(): void {
        ...
        this.errorPatterns.set(
            /network.*unreachable|failed to fetch/i,
        ...
        // Authentication errors
        this.errorPatterns.set(
            /invalid.*credential|invalid.*token/i,
        ...
        // Authorization errors
        this.errorPatterns.set(
            /insufficient.*permission|access.*denied/i,
            AuthErrorType.INSUFFICIENT_PERMISSIONS
        );
        this.errorPatterns.set(
            /account.*disabled|account.*suspended/i,
        ...
        this.errorPatterns.set(
            /suspicious.*activity|security.*violation/i,
    ...
    private initializeErrorMessages(): void {
        ...
        this.errorMessages.set(AuthErrorType.CONNECTION_TIMEOUT, {
            ...
            suggestion:
                'Try again. If the problem continues, check your internet connection or try a different network.',
        });
        ...
        this.errorMessages.set(AuthErrorType.MISSING_CLIENT_ID, {
            ...
            suggestion:
                'This is a configuration issue that needs to be resolved by the website administrator.',
        });
        ...
        this.errorMessages.set(AuthErrorType.RATE_LIMITED, {
            ...
            suggestion:
                'Please wait a few minutes before trying again. This is a security measure to protect your account.',
        });
        ...
        this.errorMessages.set(AuthErrorType.NONCE_INVALID, {
            ...
            suggestion:
                'Please refresh the page and try again. This is a security measure to protect your account.',
        });
        ...
        this.errorMessages.set(AuthErrorType.SUSPICIOUS_ACTIVITY, {
            ...
            suggestion:
                'Please wait a few minutes and try again. If you continue to have issues, please contact support.',
        });
    ...
    handleError(error: Error | string, context?: string): ErrorHandlerResponse {
        ...
        const errorConfig =
            this.errorMessages.get(errorType) ||
            this.errorMessages.get(AuthErrorType.UNKNOWN_ERROR);
...
export const shouldReportAuthError = (error: AuthError) =>
    authErrorHandler.shouldReportError(error);
export const formatAuthErrorForReporting = (error: AuthError) =>
    authErrorHandler.formatErrorForReporting(error);
