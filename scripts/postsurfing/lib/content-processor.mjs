...
export class ContentProcessor {
  ...
  async processSingleFile(filePath, contentType, lang, options) {
    ...
    if (!this.supportedExtensions.includes(extension)) {
      throw new Error(
        `Unsupported file type: ${extension}. Supported: ${this.supportedExtensions.join(', ')}`
      )
    }
  ...
  analyzeContent(content, extension) {
    ...
    analysis.extractedDescription = this.extractDescription(
  ...
  needsHtmlConversion(content, extension) {
    ...
    return (
      (startsWithDoctype && hasHtmlTag && hasHeadTag && hasBodyTag) ||
